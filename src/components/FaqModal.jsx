...
import { X, HelpCircle, ShieldCheck, Truck, Cpu, ExternalLink } from "./Icons";
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-xs leading-relaxed">
          {faqData.map((section, idx) => (
            <div key={idx} className="space-y-3">
              <h3 className="text-sm font-black uppercase tracking-wider text-[#FFDE17] pb-1 border-b border-gray-800/60">
                {section.category}
              </h3>
                ...
                {section.items.map((item, i) => (
                  <div
                    ...
                    className={`p-4 rounded-2xl border ${
                      isDarkMode ? "bg-black/30 border-gray-800/80" : "bg-gray-50 border-gray-200"
                    }`}
                  >
                    <div className="font-bold text-xs mb-1.5 flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full bg-[#FFDE17]" />
                      <span>{item.q}</span>
                    </div>
                    <p className={`pl-4 ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}>
                      {item.a}
                    </p>
                  </div>
                ))}
          ...
          >
            <div>
              <div className="font-black text-xs uppercase text-emerald-400">
            ...
            <a
              ...
              className="py-2.5 px-4 rounded-xl font-bold uppercase text-[11px] bg-[#25D366] text-black hover:bg-emerald-400 transition-colors flex items-center gap-1.5 flex-shrink-0"
            >
